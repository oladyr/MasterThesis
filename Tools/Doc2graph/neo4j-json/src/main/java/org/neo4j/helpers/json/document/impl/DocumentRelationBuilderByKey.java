...
public class DocumentRelationBuilderByKey implements DocumentRelationBuilder {
	...
	public Relationship buildRelation(Node parent, Node child, DocumentRelationContext context) {
		...
		RelationshipType relationType = RelationshipType.withName( "KEY_" + documentKey );
	 ...
	 */
	@Override
	public Set<Node> deleteRelations(DocumentRelationContext context, Transaction transaction) {
...
//		Result result = db.execute("MATCH (p)-[r:"+documentKey+"]->(c) RETURN r");
		Result result = transaction.execute("MATCH (p)-[r:" + "KEY_" + documentKey+"]->(c) RETURN r");
